/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *   Mupen64plus - LLVM JIT - Cross-platform driver                        *
...
#include <stdint.h>
#include <stdbool.h>
#if defined(LJ_SHOW_COMPILATION) || defined(LJ_SHOW_INTERPRETATION) || defined(LJ_SHOW_OPS_PER_FUNCTION)
#  include <inttypes.h>
#endif
...
/* - - - THE LLVM JIT'S CACHED INTERPRETER FALLBACK STARTS HERE - - - */
...
static void NOTCOMPILED(void)
{
	...
	if (mem != NULL)
	{
		...
		lj_recompile((uint32_t*) mem, (PC->addr - actual->start) / 4, actual);
	}
	else
...
const cpu_instruction_table lj_fallback_table = {
	LB,
	LBU,
	LH,
	LHU,
...
/* - - - THE LLVM JIT'S CACHED INTERPRETER FALLBACK ENDS HERE - - - */
...
void lj_init_page(precomp_block* page)
{
	...
	else
	{
		if (page->start >= 0x80000000 && page->end < 0xC0000000 && invalid_code[(page->start ^ 0x20000000) >> 12])
		{
			...
			lj_init_page(blocks[(page->start ^ 0x20000000) >> 12]);
		}
	}
	timed_section_end(TIMED_SECTION_COMPILER);
}

void lj_free_page(precomp_block* page)
{
	if (page->block)
	{
		free(page->block);
...
void lj_invalidate_code(enum lj_invalidate_reason reason)
{
	switch (reason)
	{
	...
	for (i = 0; i < 0x100000; i++)
	{
		if (blocks[i] && blocks[i]->block)
		{
			...
			{
				/* TODO Delete LLVM functions backing this page */
				/*
...
static uint32_t get_range_end(const uint32_t* source, const uint32_t start, precomp_block* page, n64_insn_t* insns)
{
	...
	// of a branch ending the previous page. So we only allow ONE opcode.
...
#  endif
	...
	else if (start > 0)
	{
		...
		// instructions, which is not allowed. So we only allow ONE opcode.
